...
print("STEP 3: Running REAL TimesFM on real Polymarket data")
print("=" * 60)

# Add TimesFM source to path
sys.path.insert(0, '/tmp/timesfm-install/src')
...
try:
    ...
    print(f"Version: {timesfm.__version__ if hasattr(timesfm, '__version__') else 'Unknown'}")
...
for i, market in enumerate(top_markets):
    ...
    try:
        if use_real_timesfm:
            ...
            print("   Running REAL TimesFM forecast...")
            ...
            print(f"   ✓ REAL TimesFM forecast complete")
            
            forecast_type = "REAL_TIMESFM"
...
# Save results
if results:
    output_file = "real_timesfm_forecast_results.json"
    ...
    md_file = "real_timesfm_forecast_table.md"
    with open(md_file, "w") as f:
        f.write("# TimesFM Forecasting Results\n\n")
        f.write(f"Forecast type: {results[0]['forecast_type'] if results else 'Unknown'}\n\n")
